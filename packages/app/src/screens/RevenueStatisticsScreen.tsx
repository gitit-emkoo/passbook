...
function RevenueStatisticsContent() {
  ...
  // 그래프 데이터 계산 (메모이제이션)
  const { chartData, maxRevenue, yAxisLabels } = useMemo(() => {
    const rawMaxRevenue = Math.max(...monthlyData.map(d => d.revenue), 0);
    const max = roundToNiceNumber(rawMaxRevenue);
    ...
    return { chartData: data, maxRevenue: max, yAxisLabels: uniqueLabels };
  }, [monthlyData]);
