...
interface MonthlyUsageAmountData {
  year: number;
  month: number;
  amount: number;
}
...
function UsageAmountStatisticsContent() {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [monthlyData, setMonthlyData] = useState<MonthlyUsageAmountData[]>([]);
  ...
  const loadData = useCallback(async (force = false) => {
    ...
    try {
      ...
      const data = await dashboardApi.getMonthlyUsageAmount();
      setMonthlyData(data);
      dataLastFetchedRef.current = Date.now();
    } catch (error: any) {
      console.error('[UsageAmountStatistics] load error', error);
    } finally {
      setLoading(false);
    }
  }, [monthlyData.length]);
  ...
  // 올해/지난해 누적 처리 금액 계산 (메모이제이션)
  const currentYear = useMemo(() => new Date().getFullYear(), []);
  
  const thisYearAmount = useMemo(() => {
    return monthlyData
  ...
  // 그래프 데이터 계산 (메모이제이션)
  const { chartData, maxAmount, yAxisLabels } = useMemo(() => {
    const rawMaxAmount = Math.max(...monthlyData.map(d => d.amount), 0);
    const max = roundToNiceNumber(rawMaxAmount);
    ...
    return { chartData: data, maxAmount: max, yAxisLabels: uniqueLabels };
  }, [monthlyData]);
  ...
  return (
    <Container>
      ...
      >
        <ChartCard>
          <ChartTitle>월별 처리 금액</ChartTitle>
          <ChartContainer>
            <ChartArea>
              {/* Y축 그리드 라인 */}
        ...
        <SummaryCard style={{ marginTop: 16 }}>
          <SummaryTitle>올해 처리 금액</SummaryTitle>
          <SummaryValue>{thisYearAmount.toLocaleString()}원</SummaryValue>
        ...
        <SummaryCard>
          <LastYearHeader onPress={() => setIsLastYearExpanded(!isLastYearExpanded)}>
            <LastYearHeaderLeft>
              <SummaryTitle>지난해 처리 금액</SummaryTitle>
              <SummarySubtext>
            ...
            <LastYearContent>
              {pastYearsList.map((item) => (
                <PastYearItem key={item.year}>
                  ...
                  <PastYearValue>{item.amount.toLocaleString()}원</PastYearValue>
                </PastYearItem>
              ))}
            </LastYearContent>
          )}
          {isLastYearExpanded && pastYearsList.length === 0 && (
            <LastYearContent>
              <EmptyText>지난해 처리 금액 데이터가 없습니다.</EmptyText>
            </LastYearContent>
          )}
        </SummaryCard>
      </ScrollView>
...
export default function UsageAmountStatisticsScreen() {
  return <UsageAmountStatisticsContent />;
}
