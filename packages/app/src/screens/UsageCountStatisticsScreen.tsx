...
function UsageCountStatisticsContent() {
  ...
  return (
    <Container>
      ...
      >
        <ChartCard>
          <ChartTitle>월별 처리 횟수</ChartTitle>
          <ChartContainer>
            <ChartArea>
              {/* Y축 그리드 라인 */}
        ...
        <SummaryCard style={{ marginTop: 16 }}>
          <SummaryTitle>올해 처리 횟수</SummaryTitle>
          <SummaryValue>{thisYearCount.toLocaleString()}회</SummaryValue>
        ...
        <SummaryCard>
          <LastYearHeader onPress={() => setIsLastYearExpanded(!isLastYearExpanded)}>
            <LastYearHeaderLeft>
              <SummaryTitle>지난해 처리 횟수</SummaryTitle>
              <SummarySubtext>
            ...
            <LastYearContent>
              {pastYearsList.map((item) => (
                <PastYearItem key={item.year}>
                  ...
                  <PastYearValue>{item.count.toLocaleString()}회</PastYearValue>
                </PastYearItem>
              ))}
            </LastYearContent>
          )}
          {isLastYearExpanded && pastYearsList.length === 0 && (
            <LastYearContent>
              <EmptyText>지난해 처리 횟수 데이터가 없습니다.</EmptyText>
            </LastYearContent>
          )}
        </SummaryCard>
      </ScrollView>
...
export default function UsageCountStatisticsScreen() {
  return <UsageCountStatisticsContent />;
}
